...
(function (global) {
    ...
    RHUI.openContractForm = function (id) {
        ...
        var html = '<form onsubmit="RHUI.submitContractForm(event)">' +
            ...
            '<div class="rh-field"><label>Mẫu hợp đồng</label><select id="cfContractTpl">' + selectOptions(typeof RHT !== 'undefined' ? RHT.list('CONTRACT') : (building.contractTemplates || []), 'id', 'name', c ? c.contractTemplateId : building.contractTemplateId) + '</select></div>' +
            '<div class="rh-field"><label>Mẫu hóa đơn</label><select id="cfInvoiceTpl">' + selectOptions(typeof RHT !== 'undefined' ? RHT.list('INVOICE') : (building.invoiceTemplates || []), 'id', 'name', c ? c.invoiceTemplateId : building.invoiceTemplateId) + '</select></div>' +
